...
const spreadsheet_url = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSHDGzcEMT3FkEvMxCl8PyqqadIad4_CI0QyXZvywU_Hz499cvt-zE29oMV7FvPfSVeyK2jAeMD7OUg/pub?gid=624239970&single=true&output=csv';
 ...
 * @returns {Promise<any>}
 */
const getSpreadsheetData = () => new Promise((resolve,reject) => {
    https.get(spreadsheet_url, (response) => {
        let d = '';
 ...
 * @returns {Promise<any>}
 */
const parseSpreadsheetData = data => new Promise((resolve, reject) => {
   parse(data, {}, (err, out) => {
       if (err) reject(err);
       resolve(out);
   });
 ...
 */
const constructSpreadsheetLines = data => new Promise((resolve, reject) => {
 ...
 * @returns {Promise<any>}
 */
const spreadsheetLinesToRules = lines => new Promise((resolve, reject) => {
    const types = extractTestTypes(lines[0]);
    resolve(lines.reduce((tests, line) => {
        if (line.id && line.id !== '') {
            tests.push({
                id: line.id,
            ...
            tests[tests.length - 1].tests.push(extractTestsFromLine(line, types));
        }
        return tests;
    }, []));
});
