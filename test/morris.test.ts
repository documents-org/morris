...
beau temps`,
        ...
        expect(formatted).toEqual(`le souvenir 
du\u00A0beau temps`);
    });
});
describe("A word of less than two capital letters at the end of the line is attached to the previous word", () => {
    it("in the context of brut", () => {
        const formatted = mo.apply(
            `le CROA, les 
MA et les institutions`,
            `brut`,
            17
        );
        expect(formatted).toEqual(`le CROA, les\u00A0MA 
et les institutions`);
    });
});
...
temps`,
        ...
        expect(formatted).toEqual(`le souvenir du 
beau\u00A0temps`);
    });
});
